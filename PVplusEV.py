...
ax.bar(X - offset/2, Home_elec_no_PV, color = 'tab:purple', width = offset)
ax.bar(X - offset/2, ICEV_pmt, color = 'red', bottom = Home_elec_no_PV, width = offset)
ax.bar(X - offset/2, ICEV_maint, color = 'tan', bottom = [sum(i) for i in zip(Home_elec_no_PV, ICEV_pmt)], width = offset)
ax.bar(X - offset/2, ICEV_fuel, color = 'tab:blue', bottom = [sum(i) for i in zip(Home_elec_no_PV, ICEV_pmt, ICEV_maint)], width = offset)
ax.bar(X + offset/2, Home_elec_with_PV, color = 'indigo', width = offset)
ax.bar(X + offset/2, PV_pmt, color = 'green', bottom = Home_elec_with_PV, width = offset)
ax.bar(X + offset/2, EV_pmt, color = 'maroon', bottom = [sum(i) for i in zip(Home_elec_with_PV, PV_pmt)],width = offset)
ax.bar(X + offset/2, EV_maint, color = 'orange', bottom = [sum(i) for i in zip(Home_elec_with_PV, PV_pmt, EV_pmt)], width = offset)
ax.bar(X + offset/2, EV_fuel, color = 'b', bottom = [sum(i) for i in zip(Home_elec_with_PV, PV_pmt, EV_pmt, EV_maint)],width = offset)
